...
func (c *Fullnode) Run() {
	...
	if err != nil {
		logx.Errorf("get current block height failed, error: %v",err)
	...
	if err != nil {
		logx.Errorf("get current block failed, height %d: error: %v",curHeight,err)
	...
	for {
		...
		case <-ticker.C:
			...
			if curBlock.BlockStatus > block.StatusProposing {
				...
				if err != nil {
					logx.Errorf("init new block failed, block height: %d, error: %v", curHeight, err.Error())
			...
			if err != nil {
				logx.Errorf("calculate state root failed, err:%v", err)
				panic(fmt.Sprint("calculate state root failed, err", err))
			}

			if c.bc.Statedb.StateRoot != l2Block.StateRoot {
				logx.Errorf("state root not matched between statedb and l2block: %d, local: %s, remote: %s", l2Block.Height, c.bc.Statedb.StateRoot, l2Block.StateRoot)
			...
			if err != nil {
				logx.Errorf("new block failed, block height: %d, Error: %s", l2Block.Height, err.Error())
